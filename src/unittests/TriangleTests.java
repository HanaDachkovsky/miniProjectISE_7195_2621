...
public class TriangleTests {
	...
	public void testFindIntersections() {
		...
		// ============ Equivalence Partitions Tests ==============
		// TC01:the intersection with the plane is outside the triangle and against the
		// edge
		assertNull("Ray doesn't cross the triangle",
				triangle.findIntersections(new Ray(new Point3D(5, -3, 7), new Vector(-2, 5, -4))));
		// TC02:the intersection with the plane is outside the triangle and against the
		// vertex
		assertNull("Ray doesn't cross the triangle",
				triangle.findIntersections(new Ray(new Point3D(6, 6, 7), new Vector(-9, -11, -4))));
		...
		assertNull("Ray doesn't cross the triangle",
				triangle.findIntersections(new Ray(new Point3D(6, 5, 4), new Vector(-4, -2, -1))));
		// TC05:the intersection with the plane on a vertex
		assertNull("Ray doesn't cross the triangle",
				triangle.findIntersections(new Ray(new Point3D(6, 5, 4), new Vector(-5, -3, -1))));
		// TC05:the intersection with the plane on the continuation of edge
		assertNull("Ray doesn't cross the triangle",
				triangle.findIntersections(new Ray(new Point3D(6, 5, 4), new Vector(1, 3, -1))));
	}
	...
	public void testFindGeoIntersections() {
		...
		// ============ Equivalence Partitions Tests ==============
		// TC01:1 intersection
		List<GeoPoint> result = triangle.findGeoIntersections(new Ray(new Point3D(6, 5, 4), new Vector(-4, -1, -1)), 5);
		assertEquals("Wrong number of points", 1, result.size());
		assertEquals("Ray crosses triangle", List.of(new GeoPoint(triangle, new Point3D(2, 4, 3))), result);
		// TC01:no intersections
		result = triangle.findGeoIntersections(new Ray(new Point3D(6, 5, 4), new Vector(-4, -1, -1)), 1);
